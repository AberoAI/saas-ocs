// apps/frontend/app/debug/ws-sub/WsSubClient.tsx
"use client";
...
// Bentuk minimal, menghindari `any`
...
export default function WsSubClient() {
  // ---- tRPC subscription (gabungan fungsionalitas keduanya) ----
  ...
  // Log ringan saat data baru—ini bagian yang dulu ada di ws-sub.tsx
  useEffect(() => {
    if (last) {
  ...
  // ---- Diag: native WebSocket (stabil jangka panjang) ----
  ...
  return (
    <div className="space-y-3 p-6">
      ...
      {status === "error" && (
        <pre className="mt-2 whitespace-pre-wrap text-red-600">{errMsg}</pre>
      )}
